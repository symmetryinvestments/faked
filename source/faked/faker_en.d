...
@safe:
	...
	string commerceDepartment() {
		const string[] strs =
		[ q"{Books}", q"{Movies}", q"{Music}", q"{Games}", q"{Electronics}", q"{Computers}", q"{Home}", q"{Garden}"
		, q"{Tools}", q"{Grocery}", q"{Health}", q"{Beauty}", q"{Toys}", q"{Kids}", q"{Baby}", q"{Clothing}"
		, q"{Shoes}", q"{Jewelery}", q"{Sports}", q"{Outdoors}", q"{Automotive}", q"{Industrial}" ];
	...
	string hackerPhrase() {
		switch(uniform(0, 8, this.rnd)) {
			...
			case 3: return "You can't " ~ hackerVerb() ~ " the " ~ hackerNoun() ~ " without " ~ hackerIngverb() ~ " the " ~ hackerAdjective() ~ " " ~ hackerAbbreviation() ~ " " ~ hackerNoun() ~ "!";
			case 4: return "Use the " ~ hackerAdjective() ~ " " ~ hackerAbbreviation() ~ " " ~ hackerNoun() ~ ", then you can " ~ hackerVerb() ~ " the " ~ hackerAdjective() ~ " " ~ hackerNoun() ~ "!";
